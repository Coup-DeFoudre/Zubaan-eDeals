    ...
    for (let item of products) {
      if (item.title in stickers) {
        if (item.availableQty > 0) {
          if (
            !stickers[item.title].color.some(
              (color) => color.name === item.color
            )
          ) {
            stickers[item.title].color.push({ name: item.color });
          }
          if (
            !stickers[item.title].size.some((size) => size.name === item.size)
          ) {
            stickers[item.title].size.push({ name: item.size });
          }
        }
      } else {
